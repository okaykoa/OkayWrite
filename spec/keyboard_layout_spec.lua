...
describe("keyboard_layout.resolve (fr)", function()
  ...
  it("maps the ² key (ANSI backtick position, no distinct shifted glyph)", function()
    assert.are.equal("²", r("`"))
    assert.are.equal("²", r("`", true))
  end)
end)
...
describe("keyboard_layout.resolve (es)", function()
  ...
  it("maps the ç key (ANSI backslash position, legacy Spanish typewriter key)", function()
    assert.are.equal("ç", r("\\"))
    assert.are.equal("Ç", r("\\", true))
  end)
end)
